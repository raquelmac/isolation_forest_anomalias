...
Faça um estudo acerca dos valores de parcelas do bolsa família, para identificar possíveis fraudes.

Apresente informações que possam auxiliar a investigação, como a distribuição dos valores de possíveis fraudes e dados dessas  pessoas.
...
try:
    ...
    df_bf_pl = pl.read_parquet(ENDERECO_DADOS + 'dados_bolsa_familia_anomalias.parquet')  
...
try:
    ...
    with pl.StringCache(): # Acelerando os trabalhos Trabalhar com processamento em larga escala
        ...
        modelo = IsolationForest(
                                ...
                                random_state=42,
                                n_estimators=100, #número de árvores (subconjuntos) qnto mais árvores mais precisa é a detecção, porém leva mais tempo de processamento
        ...
        df_bf_pl = df_bf_pl_lazy.filter(pl.col('anomalia') == -1)
...
try:
    ...
    plt.suptitle('Detecção de anomalias no valor parcela')
